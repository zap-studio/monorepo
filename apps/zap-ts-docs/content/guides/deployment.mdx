...
There is no single command that deploys everything.
...
Two packages already carry the Vercel versions of the parts that differ.

**Background jobs.** Use `@zap-ts/queues-vercel` instead of
`@zap-ts/queues-cloudflare`. Your job definitions do not change. They live in
`@zap-ts/queues`, which knows about neither host. Only the adapter changes. See
[Queues](/packages/queues).

**Error reporting for the API.** `@zap-ts/observability` ships both
`./hono/cloudflare` and `./hono/vercel`. Import the Vercel one. See
[Observability](/packages/observability).

Two more things change.

**The database connection.** Hyperdrive is a Cloudflare service, so it does not
apply. Point `DATABASE_URL` at your Postgres directly, and use a connection
pooler from your database host if it offers one.

**Settings and keys.** The `deploy:*` scripts run `varlock-wrangler`, which is
the Cloudflare path. On Vercel, set your values in the Vercel project settings,
or use `varlock` with Vercel's own CLI. Your `.env.schema` files stay exactly as
they are: they describe what the app needs, not where it runs.
