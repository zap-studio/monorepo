...
The `deploy:*` scripts run `varlock-wrangler`, which is the Cloudflare path. It
does two things at once: it resolves your values, and it ships them to the host.
On Vercel you keep the first half and change the second:

```json
// apps/web/package.json
"deploy": "varlock run -- vercel deploy --prod"
```

`varlock run` resolves your `.env.schema` and `.env.local` the same way it
always does, so the build sees the same values. What differs is where the
values live at runtime: set them in the Vercel project settings, or push them
with `vercel env add`.

Nothing keeps the Vercel dashboard and your files in step, which is the one
thing you lose. On Cloudflare, varlock overwrites the vars and secrets on every
deploy. On Vercel, a value you add by hand in the dashboard stays until you
remove it.
