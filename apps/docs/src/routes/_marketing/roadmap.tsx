...
import { FadeIn, StaggerContainer, StaggerItem } from "@/routes/_marketing/-components/animated";
...
const milestones: Milestone[] = [
  ...
  },
  {
    ...
    description:
      "Ship the logging package, with structured logs, type-safe context, and transport-agnostic output handling.",
    status: "planned",
  },
  {
    ...
    description:
      "Ship the events package, with typed event contracts and lightweight pub/sub primitives for app workflows.",
    status: "planned",
  },
  {
  ...
  },
  {
    ...
    description:
      "Ship the realtime package, with lightweight, framework-agnostic real-time event subscriptions built on standard primitives.",
    status: "planned",
  },
  {
    ...
    title: "Zap.ts v0.1",
    description:
      "Release v0.1 of Zap.ts, an opinionated, AI-first full-stack framework scoped to SaaS and built on top of the Zap Studio package ecosystem.",
    status: "planned",
  },
];
...
export const Route = createFileRoute("/_marketing/roadmap")({
  head: () => ({
    meta: pageMeta("Roadmap", roadmapDescription),
  }),
        ...
        <StaggerContainer className="flex flex-col" delay={0.1} stagger={0.07}>
          {milestones.map((milestone, index) => (
            <StaggerItem key={milestone.title}>
              <MilestoneRow isLast={index === milestones.length - 1} milestone={milestone} />
            </StaggerItem>
          ))}
        </StaggerContainer>
      </section>
