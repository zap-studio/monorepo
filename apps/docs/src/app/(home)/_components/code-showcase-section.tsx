...
export async function CodeShowcaseSection(): Promise<ReactNode> {
  const highlighted = await highlight(MULTI_PACKAGE_EXAMPLE, {
    lang: "ts",
    themes: {
      dark: "github-dark",
      light: "github-light",
    ...
    components: {
      pre: (props) => (
        ...
        >
          <Pre
            {...props}
            className="whitespace-pre-wrap break-words font-mono text-[12px] leading-[1.75] sm:text-[13px]"
          />
        </CodeBlock>
      ),
    },
