...
`api.get`, `api.post`, `api.put`, `api.patch`, and `api.delete` — HTTP method helpers bound to `$fetch`. Use them when the HTTP method is known up front and the response should be validated.
