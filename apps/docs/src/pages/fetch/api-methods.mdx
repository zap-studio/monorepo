...
The `api` object exposes method-bound `$fetch` helpers. Use them when the HTTP method is known up front and the response should be validated.
...
Each helper has the same overloads, options, and error behavior as [`$fetch`](/fetch/fetch-function), but injects its HTTP method for you. The injected method always wins — a `method` passed in `options` is overridden.

:::note The schema argument always validates the **response** body. It does not validate `json` or `body` request payloads. :::
...
For clarity, prefer [`$fetch`](/fetch/fetch-function) directly when raw response handling is the main goal.
