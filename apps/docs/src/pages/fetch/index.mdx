...
- **[`Result`-returning variant](/fetch/result)** through `$fetchResult`/`apiResult`, for explicit error handling with [`@zap-studio/monads`](/monads) instead of throw/catch.
...
- [Result-Returning Variant](/fetch/result) — `$fetchResult`/`apiResult`.
