...
[`fetch`](/fetch) validates responses through [`@zap-studio/validation`](/validation), so any Standard Schema-compatible validator can be used.
...
For validation outside HTTP requests, use [`@zap-studio/validation`](/validation) directly.
