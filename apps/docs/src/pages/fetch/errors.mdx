...
See [Result-Returning Variant](/fetch/result) for `$fetchResult`/`apiResult`, an alternative to try/catch that returns these same errors as `Err` instead of throwing them.
