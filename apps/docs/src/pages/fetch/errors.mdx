...
| --- | --- | --- |
| `FetchError` | `@zap-studio/fetch/errors` | The response is not ok and `throwOnFetchError` is `true` (default). |
| `ValidationError` | `@zap-studio/validation/errors` | A schema is provided, validation returns issues, and `throwOnValidationError` is `true` (default). |
