...
`createFetch(...)` creates a fetch client with shared `baseURL`, headers, query params, and error defaults. It returns an independent `$fetch` function and `api` method set bound to those defaults, plus [`$fetchResult`/`apiResult`](/fetch/result) for the `Result`-returning API.
