...
Use `createFetch` to configure shared defaults once and reuse them across requests. It returns an independent `$fetch` function and `api` method set bound to those defaults.
