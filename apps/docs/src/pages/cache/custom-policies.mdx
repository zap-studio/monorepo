...
## Example: MRU (Most Recently Used)

Evicts the most-recently-used key instead of the least — useful for access patterns where the most recent entry is the least likely to be reused (e.g. scanning a large sequence once).
...
- [Eviction Policies](/cache/eviction-policies) — the built-in `lru()`, `lfu()`, `fifo()` for reference implementations
