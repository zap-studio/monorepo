...
`@zap-studio/monads` is a deliberately small v1: `Result`, `ResultAsync`, and `Option`, as standalone, tree-shakeable functions composed with `pipe` — not class instances with chained methods.
