...
Color is skipped automatically when it wouldn't render correctly: only on a real TTY (and not when `NO_COLOR` is set) in Node/Bun, and unconditionally colored anywhere without a `process` global — browsers and Workers — on the assumption it's a devtools-like console.
...
A `LogRecord` has `level`, `message`, `context` (`Record<string, unknown> | undefined`), and `timestamp` (a `Date`, built fresh for each call that passes the `minLevel` filter). A `LogFormatter` returns the argument list `console[method](...)` gets called with — return one string for a single line, or more arguments (like `classicFormat` and `prettyFormat` do) to hand `context` off for native inspection.
