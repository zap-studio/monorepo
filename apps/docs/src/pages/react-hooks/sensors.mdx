...
[`navigator.userAgentData`](https://developer.mozilla.org/en-US/docs/Web/API/User-Agent_Client_Hints_API)'s low-entropy fields (`brands`, `mobile`, `platform`) — a structured, Chromium-only replacement for parsing `navigator.userAgent` (see [browser compatibility](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/userAgentData#browser_compatibility)). A static device capability — doesn't change at runtime. `undefined` — the SSR-safe default — where User-Agent Client Hints is unsupported.
...
[`navigator.virtualKeyboard`](https://developer.mozilla.org/en-US/docs/Web/API/VirtualKeyboard_API)'s `boundingRect` (Chromium-only VirtualKeyboard API — see [browser compatibility](https://developer.mozilla.org/en-US/docs/Web/API/VirtualKeyboard_API#browser_compatibility)), updating on its `geometrychange` event. Falls back to `{ x: 0, y: 0, width: 0, height: 0 }` during server rendering, before the client subscribes, and where the API is unsupported.
