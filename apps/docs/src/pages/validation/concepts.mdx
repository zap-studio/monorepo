...
:::info[Note] Standard Schema is a shared initiative across major validation libraries and ecosystem maintainers, including [Zod](https://zod.dev/), [Valibot](https://valibot.dev/), and [ArkType](https://arktype.io/). :::
...
- The **factories** (`createStandardValidator`, `createStandardValidatorSync`) bind a schema once and return a reusable validator function.
...
You bring the schema library; the package gives you one predictable way to run it and handle its failures. Continue with [Async Validation](/validation/async-validation) to see the helpers in action.
