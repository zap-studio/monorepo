...
Optional throwing behavior via `throwOnError`, backed by a shared `ValidationError` class. Instead of dealing with each schema library's error shape, you catch one predictable error class and access normalized [Standard Schema](https://standardschema.dev) `issues`.
