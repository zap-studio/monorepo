...
At Zap Studio, multiple packages need runtime validation ([`fetch`](/fetch), [`permit`](/permit), and others). We wanted one internal validation flow that works with any Standard Schema-compatible library.
