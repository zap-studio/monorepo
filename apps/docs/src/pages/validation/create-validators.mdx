...
If the schema validates asynchronously, the returned validator throws an `Error` with the message `Async schemas are not supported by createSyncStandardValidator`. The underlying `TypeError` is attached as the error's `cause`.
:::
