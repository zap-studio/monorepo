...
description: Build reusable validation functions with createStandardValidator and createSyncStandardValidator.
---
...
`createSyncStandardValidator(schema)` returns a synchronous validator. It supports the same options and return modes as `standardValidateSync`.
...
:::warning[Async Schemas] If the schema validates asynchronously, the returned validator throws an `Error` with the message `Async schemas are not supported by createSyncStandardValidator`. The underlying `TypeError` is attached as the error's `cause`. :::
...
- For one-shot validation without a reusable function, call [`standardValidate` or `standardValidateSync`](/validation/how-to-validate) directly.
