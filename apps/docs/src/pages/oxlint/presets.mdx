...
`tanstack-query` above already bundles `@tanstack/eslint-plugin-query`. `@tanstack/eslint-plugin-router` is separate — add it if the project uses TanStack Router:
