---
title: OpenTelemetry
description: Every createEnv validation pass surfaced as an env.validate span through @opentelemetry/api.
---
...
Each `createEnv` validation pass gets an `INTERNAL` span named `env.validate`. On failure, the span carries the invalid key names — never their values — as the `env.invalid_keys` attribute, and is marked as an error, with the thrown error recorded via `recordException`.

Validation itself runs once at startup, not per-request, so the span cost is negligible.

## Skipped Validation, No Span

When [`skipValidation`](/env/advanced-options#skipvalidation) is `true`, `createEnv` returns early before the `env.validate` span is created — there's nothing to trace, since no validation actually runs.
...
- [Errors](/env/errors) — `EnvValidationError`, whose `invalidKeys` end up on the span
