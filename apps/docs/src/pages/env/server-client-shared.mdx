...
`client` vars need `runtimeEnvStrict`, with each key on its own `process.env.X` line. See [Advanced Options](/env/advanced-options#runtimeenvstrict) for why a plain `runtimeEnv: process.env` does not survive bundling.
