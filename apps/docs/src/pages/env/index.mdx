...
Instead of a live preview through a dev server, `@zap-studio/env` takes a simpler, static approach: [`generateEnvExample`](/env/generate-env-example) builds a `.env.example` file straight from your schema. No dev server needed, so it's safe to run in CI and commit.
...
- **[`.env.example` generation](/env/generate-env-example)** via `generateEnvExample(...)`.
- **[Structured errors](/env/errors)**: `EnvError`, `EnvValidationError`, and `EnvAccessError`.
...
- [`generateEnvExample`](/env/generate-env-example) — schema-driven `.env.example` generation
- [Errors](/env/errors) — `EnvError`, `EnvValidationError`, `EnvAccessError`
