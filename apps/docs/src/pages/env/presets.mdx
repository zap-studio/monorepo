---
title: Presets
description: Ready-made EnvSchema objects for env vars that hosting platforms set automatically.
---

# Presets

Presets cover env vars that a hosting platform sets on its own — deployment IDs, git metadata, region, and so on. Compose one into `createEnv` (or `generateEnvExample`) via [`extends`](/env/extends) to type and validate a platform's vars alongside your app's own:
...
Presets compose like any other `EnvSchema` — pass more than one to `extends` if your app can run on more than one platform:
