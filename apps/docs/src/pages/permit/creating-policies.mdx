...
:::info Define resources and actions in a centralized location to ensure consistency and type safety across your application. :::
...
:::warning Use `as const` so TypeScript infers literal types for actions. This enables autocomplete and type checking when writing rules and permission strings. Learn more about [`const` assertions](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#const-assertions). :::
...
:::info Context is just a type — it can be anything relevant to your application's authorization needs. Include all the data your rules need so they never have to fetch anything. :::

## Creating a Policy

Use `createPolicy()` to combine your resources, actions, and rules into a policy:
...
const policy = createPolicy<AppContext>({
  ...
  rules: {
    ...
    comment: {
      ...
      delete: when(
        (ctx, _action, comment) => ctx.user?.id === comment.authorId
      ),
    },
  },
});
...
:::note `createPolicy()` throws a [`PolicyError`](/permit/errors) at creation time if a resource key is missing its schema. Rules for individual actions are optional — an action without a rule is always denied. :::
...
const canEdit = await policy.can(context, "post:write", post);
...
const canOtherEdit = await policy.can(otherContext, "post:write", post);
...
- [Policy Rules](/permit/policy-rules) — Learn the `allow()`, `deny()`, and `when()` rule builders
- [Conditions](/permit/conditions) — Compose rules with `and()`, `or()`, `not()`, and `has()`
