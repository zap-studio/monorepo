...
description: Combine multiple policies with AND or OR strategies in permit.
---
...
Merged policies evaluate in parallel and never short-circuit: every policy is invoked regardless of outcome. `mergePoliciesAnd()` requires all results to allow, `mergePoliciesOr()` requires at least one — but both wait for every policy's `can()` to settle first.
...
const filePolicy = mergePoliciesOr(
  ownerPolicy,
  publicPolicy,
  sharedPolicy,
  teamPolicy
...
const securedPolicy = mergePoliciesAnd(
  auditPolicy,
  compliancePolicy,
  ratePolicy
);
