...
`permit` merges policies via `mergePoliciesEvery` and `mergePoliciesSome`. As your application grows, you may want to organize authorization logic into separate policies. `permit` provides two merge strategies: deny-overrides and allow-overrides.
