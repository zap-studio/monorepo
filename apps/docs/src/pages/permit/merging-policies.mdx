...
:::note Merged policies evaluate sequentially and short-circuit: `mergePoliciesEvery()` stops at the first deny, `mergePoliciesSome()` stops at the first allow. :::
...
5. **Order for short-circuiting** — Put policies that deny (or allow) most often first
