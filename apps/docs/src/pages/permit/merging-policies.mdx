...
As your application grows, you may want to organize authorization logic into separate policies. `permit` provides two merge strategies: deny-overrides and allow-overrides.
...
:::note Merged policies evaluate sequentially and short-circuit: `mergePolicies()` stops at the first deny, `mergePoliciesAny()` stops at the first allow. :::
...
const filePolicy = mergePoliciesAny(
  ownerPolicy,
  publicPolicy,
  sharedPolicy,
  teamPolicy
...
```ts
const emptyAnd = mergePolicies(); // can() always resolves to false
const emptyOr = mergePoliciesAny(); // can() always resolves to false
```
