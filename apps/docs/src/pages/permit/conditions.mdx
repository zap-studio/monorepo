---
title: Conditions
description: Build complex authorization rules using and(), or(), not(), and has() condition combinators in permit.
---

# Conditions

Conditions compose via `and`, `or`, and `not`. They are the building blocks for creating complex authorization rules — `permit` provides combinators to compose simple conditions into powerful, readable policies.
...
const policy = createPolicy<AppContext>({
  ...
  rules: {
    post: {
      ...
      report: when(
        and(isNotOwner, (ctx, _, post) => !post.reportedBy.includes(ctx.user?.id ?? "")),
      ),
    },
  },
});
...
The `has()` helper creates a condition that checks if a context property equals a specific value. It compares with strict equality (`===`).
...
const resources = {
  settings: z.object({ id: z.string() }),
  billing: z.object({ id: z.string() }),
  export: z.object({ id: z.string() }),
} satisfies Resources;
...
`has()` only checks top-level context properties. For nested values like `ctx.user.role`, write the condition inline: `when((ctx) => ctx.user?.role === "admin")`.
...
const isAdmin: ConditionFn<DocContext, string, Document> = (ctx) => ctx.user?.role === "admin";

const isArchived: ConditionFn<DocContext, string, Document> = (_, __, doc) =>
  doc.status === "archived";

const isLocked: ConditionFn<DocContext, string, Document> = (_, __, doc) => doc.isLocked;

// Composed conditions
const canEdit = and(or(isOwner, isAdmin), not(isArchived), or(not(isLocked), isAdmin));
