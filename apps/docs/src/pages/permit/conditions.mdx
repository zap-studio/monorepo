...
Via `and`, `or`, and `not`. Conditions are the building blocks for creating complex authorization rules — `permit` provides combinators to compose simple conditions into powerful, readable policies.
...
:::note `has()` only checks top-level context properties. For nested values like `ctx.user.role`, write the condition inline: `when((ctx) => ctx.user?.role === "admin")`. :::
