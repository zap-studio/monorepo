...
:::note `PolicyError` is exported from the package root, and also from the `@zap-studio/permit/errors` subpath for granular imports. :::
...
```ts
import { assertNever } from "@zap-studio/permit";
