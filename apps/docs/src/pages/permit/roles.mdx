...
const policy = createPolicy<AppContext>({
  ...
  rules: {
    post: {
      ...
      read: when(
        (ctx, _, post) => post.status === "published" || ctx.role === "user"
      ),
      // Users can write their own posts
      write: when(
        (ctx, _, post) => ctx.role === "user" && ctx.user?.id === post.authorId
      ),
    ...
    user: {
      ...
      read: when(
        or((ctx, _, user) => ctx.user?.id === user.id, hasRole("admin"))
      ),
...
console.log(
  await multiRolePolicy.can(betaPremiumUser, "betaFeature:access", feature)
); // true (has beta-tester)
console.log(
  await multiRolePolicy.can(betaPremiumUser, "supportTicket:escalate", ticket)
); // true (has premium)
```
