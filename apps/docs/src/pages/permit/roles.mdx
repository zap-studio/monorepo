...
Role-Based Access Control (RBAC) is a common authorization pattern where permissions are assigned to roles, and users are assigned roles. `permit` provides utilities for implementing RBAC with support for role hierarchies.
...
:::note `hasRole()` returns a `ConditionFn` for use inside `when()` (or combinators like `or()`). It does not return a boolean directly. :::
