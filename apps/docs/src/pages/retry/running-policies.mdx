...
See [Non-throw Mode](/retry/non-throw-mode) for `throwOnExhausted: false`, [Result-Returning Variant](/retry/result) for `runRetryPolicyResult`, and [Structured Errors](/retry/errors) for what `runRetryPolicy(...)` throws by default.
