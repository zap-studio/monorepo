...
- **[`Result`-returning variant](/retry/result)** via `runRetryPolicyResult(policy, execute, options?)`, for explicit error handling with [`@zap-studio/monads`](/monads) instead of throw/catch.
...
- [Result-Returning Variant](/retry/result)
