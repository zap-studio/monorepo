...
See [Non-throw Mode](/retry/non-throw-mode) for how these same errors surface as a `result.error` instead of being thrown, or [Result-Returning Variant](/retry/result) for `Result`/`ResultAsync` from `@zap-studio/monads`.
