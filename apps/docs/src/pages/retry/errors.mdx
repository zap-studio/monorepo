...
The default `onExhausted` used by `runRetryPolicy(...)` (when a policy omits its own) returns a `RetryError` with the message `"Retry policy exhausted all attempts."` — the built-in `fixedDelay` and `exponentialBackoff` policies rely on this default. When retries are exhausted in the default throw mode, `runRetryPolicy(...)` throws that error.
