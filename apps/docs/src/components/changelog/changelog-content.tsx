...
function HighlightedCodeBlock({
  ...
  return (
    <pre
      className={`${nested ? "mt-2" : ""} overflow-x-auto rounded-lg border border-fd-border bg-fd-card/40 p-4 font-mono text-[13px] leading-6 shadow-none`}
    >
      <code className={`language-${normalizeLanguage(language)}`}>{code}</code>
    </pre>
  );
}
