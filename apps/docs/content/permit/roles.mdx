...
description: Implement RBAC with role hierarchies and multi-role support using hasRole() in permit.
---

# Role-Based Access Control

`permit` implements role checks via `hasRole(role, hierarchy?)`, with inheritance resolved by `collectInheritedRoles`. Role-Based Access Control (RBAC) is a common authorization pattern where permissions are assigned to roles, and users are assigned roles.
...
`hasRole()` returns a `ConditionFn` for use inside `when()` (or combinators like `or()`). It does not return a boolean directly.

:::

### Context Requirements

Your context must include a top-level `role` property that can be:
...
const policy = createPolicy<AppContext>({
  ...
  rules: {
    post: {
      ...
      read: when((ctx, _, post) => post.status === "published" || ctx.role === "user"),
      // Users can write their own posts
      write: when((ctx, _, post) => ctx.role === "user" && ctx.user?.id === post.authorId),
      // Moderators and admins can delete any post
      delete: when(or(hasRole("moderator"), hasRole("admin"))),
      // Only admins can publish
      publish: when(hasRole("admin")),
    },
    user: {
      // Users can read their own profile, admins can read any
      read: when(or((ctx, _, user) => ctx.user?.id === user.id, hasRole("admin"))),
      // Users can update their own profile
      update: when((ctx, _, user) => ctx.user?.id === user.id),
      // Only admins can delete users
      delete: when(hasRole("admin")),
      // Moderators and admins can ban users
      ban: when(or(hasRole("moderator"), hasRole("admin"))),
    },
  },
});
```
...
```ts
import type { RoleHierarchy } from "@zap-studio/permit";
...
import { z } from "zod";
import { createPolicy, when, hasRole, or } from "@zap-studio/permit";
import type { Resources, Actions, RoleHierarchy } from "@zap-studio/permit";
...
const multiRolePolicy = createPolicy<MultiRoleContext>({
  ...
  rules: {
    ...
    supportTicket: {
      ...
      escalate: when(
        or(hasRole("premium", roleHierarchy), hasRole("support-agent", roleHierarchy)),
      ),
    },
  },
});
...
console.log(await multiRolePolicy.can(betaPremiumUser, "betaFeature:access", feature)); // true (has beta-tester)
console.log(await multiRolePolicy.can(betaPremiumUser, "supportTicket:escalate", ticket)); // true (has premium)
```

## Collecting Inherited Roles

The `collectInheritedRoles()` function returns the full set of roles a user has, including inherited ones. It handles repeated roles safely and returns a `Set`.
...
type Role = "guest" | "user" | "moderator" | "admin" | "beta-tester";
...
const mixedRoles = collectInheritedRoles(["user", "beta-tester"], hierarchy);
