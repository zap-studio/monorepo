...
<Callout type="info">
  You can change this `NotificationOptions` interface however you want and modify the `notify` and
  `notifyForced` helpers to add more fields according to the [Tauri notification plugin
  options](https://v2.tauri.app/reference/javascript/notification/#sendnotification).
</Callout>
