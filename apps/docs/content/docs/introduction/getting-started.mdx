...
Zap.ts follows a classic **Next.js** app structure with a `src/` directory. Everything is explained in the [best practices](/docs/misc/best-practices) guide.

<Files>
  <Folder name="actions">
    <File name="server-action.ts" />
  </Folder>
...
In addition, Zap.ts gives you additional plugins that follows a modular approach, since each plugin is encapsulated in its own directory under `zap/`.
