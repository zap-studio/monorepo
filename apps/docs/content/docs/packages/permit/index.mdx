...
const policy = createPolicy<AppContext>({
  ...
  rules: {
    post: {
      delete: when(or(hasRole("admin"), (ctx, _, post) => ctx.user.id === post.authorId)),
    },
  },
});
...
app.delete("/posts/:id", async (req, res) => {
  ...
  if (!(await policy.can(req.context, "delete", "post", post))) {
    return res.status(403).json({ error: "Forbidden" });
  }
...
if (await policy.can(context, "read", "post", post)) {
