...
Policy rules are functions that determine whether an action is allowed or denied. `permit` provides three rule builders: `allow()`, `deny()`, and `when()`.
...
Rules can be combined with condition combinators. See the [Conditions](/docs/packages/permit/conditions) page for details on using `and()`, `or()`, and `not()`.
