...
<Callout type="info">
  We recommend defining resources and actions in a centralized location to
...
<Callout type="warn">
  You should use `satisfies` to ensure type safety and consistency. Learn more
  about the [TypeScript `satisfies`
...
<Callout type="warn">
  Use `as const` to ensure TypeScript infers literal types for actions. This
  enables autocomplete and type checking when writing rules. Learn more about
  the [TypeScript `as const`
...
<Callout type="info">
  Context is just a type that represents the runtime information available when
  checking permissions. It can be anything relevant to your application's
  authorization needs. Providing context will make your experience way better.
