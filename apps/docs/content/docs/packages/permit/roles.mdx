...
<Callout type="warn">
  For now, `hasRole()` returns a `ConditionFn`, which can be used in `when()` conditions. Thus, it
  doesn't return a boolean value directly. Prefer using `role` directly in conditions.
</Callout>
...
console.log(await multiRolePolicy.can(betaPremiumUser, "access", "betaFeature", feature)); // true (has beta-tester)
console.log(await multiRolePolicy.can(betaPremiumUser, "escalate", "supportTicket", ticket)); // true (has premium)
```
