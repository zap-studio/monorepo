---
title: API Methods
description: Convenient HTTP verb methods on the api object with automatic validation and full type inference.
---
...
async function getUserProfile(userId: string) {
  const profile = await api.get(`https://api.example.com/users/${userId}`, UserProfileSchema);
...
async function updateSettings(
  ...
  return api.put(`https://api.example.com/users/${userId}/settings`, UserSettingsSchema, {
    body: settings,
  });
}
...
async function updateProfile(
  ...
  return api.patch(`https://api.example.com/users/${userId}/profile`, ProfileSchema, {
    body: updates,
  });
}
...
async function deletePost(postId: string) {
  const result = await api.delete(`https://api.example.com/posts/${postId}`, DeleteResponseSchema);
...
async function removeFromCart(cartId: string, productId: string) {
  ...
  return api.delete(`https://api.example.com/carts/${cartId}/items/${productId}`, CartSchema);
}
```
...
The `api.*` methods always require a schema for validation. If you need raw responses without validation, use [`$fetch`](/docs/packages/fetch/fetch-function) directly.
