...
async function $fetch(resource: string, options?: ExtendedRequestInit): Promise<Response>;
```
...
| ------------------------ | ------------------------------------------- | ------- | ----------------------------------------------------- |
| `body`                   | `BodyInit \| JsonValue`                     | -       | Request body (auto-stringified for plain JSON values) |
