...
description: Handle HTTP and validation errors with FetchError and ValidationError in fetch.
---

`fetch` provides two specialized error classes for granular error handling: `FetchError` for HTTP errors and `ValidationError` for schema validation failures.
...
type FetchResult<T> = { success: true; data: T } | { success: false; error: string; code: string };
...
```typescript
type Result<T> = { value: T; issues?: undefined } | { value?: undefined; issues: Issue[] };
```
