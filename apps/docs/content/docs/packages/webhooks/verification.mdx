...
[`@zap-studio/webhooks`](/docs/packages/webhooks) exports `createHmacVerifier` from `@zap-studio/webhooks/verify`. It is runtime-agnostic: the helper uses the Web Crypto API rather than Node `crypto`, so it works anywhere `globalThis.crypto.subtle` is available.
