...
At Zap Studio, multiple packages need runtime validation ([`fetch`](/docs/packages/fetch), [`permit`](/docs/packages/permit), and others). We wanted one internal validation flow that works with any Standard Schema-compatible library.
