...
describe("createCache get/set/has", () => {
...
describe("createCache capacity and eviction", () => {
  ...
  it("does not evict when updating an existing key at capacity", () => {
    ...
    cache.set("a", 1);
    cache.set("b", 2);
    // oxlint-disable-next-line sonarjs/no-element-overwrite -- deliberate: re-setting "a" is what this test verifies (an update must not evict).
    cache.set("a", 10);
  ...
  it("does not remove any entry when the policy has nothing to evict", () => {
    const policy: EvictionPolicy<string> = {
      ...
      onGet: vi.fn<(key: string) => void>(),
      onSet: vi.fn<(key: string) => void>(),
    };
    const onEvict = vi.fn<(key: string, value: number) => void>();
...
describe("createCache size/capacity", () => {
...
describe("createCache EvictionPolicy contract usage", () => {
  it("calls onGet on the configured policy for a live hit", () => {
    const onGet = vi.fn<(key: string) => void>();
    const policy: EvictionPolicy<string> = {
      clear: vi.fn<() => void>(),
      evict: () => undefined,
      onDelete: vi.fn<(key: string) => void>(),
