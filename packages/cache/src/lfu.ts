 ...
 * ```
 */
export const lfu = <K>(): EvictionPolicy<K> => {
  ...
  return {
    ...
    evict(): K | undefined {
      ...
      let bucket = bucketsByFrequency.get(minFrequency);
      while (bucket === undefined || bucket.size === 0) {
        minFrequency += 1;
        bucket = bucketsByFrequency.get(minFrequency);
      }
      ...
      if (bucket.size === 0) {
        bucketsByFrequency.delete(minFrequency);
      }
