...
- First release. `createStore(initialState, actionsFactory?, options?)` returns `.get()` (state and actions together), `.getState()` (state only), and `.subscribe(listener)` (returns a plain unsubscribe function).
