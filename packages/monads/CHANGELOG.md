...
- `Result<T, E>`: `ok()`, `err()`, `isOk()`, `isErr()`, `fromThrowable()`, and the `Result` namespace (`map`, `mapErr`, `andThen`, `orElse`, `unwrapOr`, `unwrapOrElse`, `unwrap`, `match`).
- `ResultAsync<T, E>`: a thenable wrapper around `Promise<Result<T, E>>` with chainable `map()`, `mapErr()`, `andThen()`, `orElse()`, `match()`, plus `fromPromise()`.
