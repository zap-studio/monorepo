...
describe("BaseWaitlistSDK", () => {
  let adapter: MockAdapter;
  let eventBus: EventBus;
  let sdk: BaseWaitlistSDK;

  beforeEach(() => {
    adapter = new MockAdapter();
    eventBus = new EventBus();
    sdk = new BaseWaitlistSDK({ adapter, events: eventBus });
  });

  describe("constructor", () => {
    it("should create an instance with adapter and events", () => {
      expect(sdk).toBeInstanceOf(BaseWaitlistSDK);
    });

    it("should create a default EventBus if not provided", () => {
      const sdkWithoutEvents = new BaseWaitlistSDK({ adapter });
      expect(sdkWithoutEvents).toBeInstanceOf(BaseWaitlistSDK);
    });
  });
  ...
  describe("error handling", () => {
    it("should handle adapter errors gracefully", async () => {
      ...
      const errorSdk = new BaseWaitlistSDK({ adapter: errorAdapter });
