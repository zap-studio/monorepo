...
const createSensorMock = (reading: { x: number; y: number; z: number }) => {
  // SAFETY: the lines right below set every field of the intersection type (activated, onerror, onreading, start, stop, x, y, z), so this EventTarget really is a full Magnetometer-shaped mock before any test reads it.
