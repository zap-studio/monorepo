...
`@zap-studio/retry` builds the safe behavior in by default. `exponentialBackoff` and `linearBackoff` come with jitter (`"full"` or `"equal"`, the same strategies AWS recommends) already wired in, and every retry loop respects a passed-in `AbortSignal`, checked before, between, and during attempts. You get retry policies as values you configure once and reuse, instead of logic you have to get right from scratch in every project.
