...
describe("fetchInternal", () => {
  ...
  describe("error handling", () => {
    it("should throw FetchError on non-ok response when throwOnFetchError is true", async () => {
      fetchMock.mockResolvedValue(new Response("Bad", { status: 500, statusText: "Server Error" }));
      await expect(
    ...
    it("should not throw on non-ok response when throwOnFetchError is false", async () => {
      fetchMock.mockResolvedValue(new Response("Bad", { status: 404, statusText: "Not Found" }));
  ...
  describe("response handling", () => {
    ...
    it("should return validated value when throwOnValidationError is true", async () => {
      fetchMock.mockResolvedValue(new Response(JSON.stringify({ val: "x" }), { status: 200 }));
      const schema = {
        "~standard": {
          version: 1,
          vendor: "standard-schema",
...
describe("URL parsing with hash and special characters", () => {
  ...
  describe("hash/fragment preservation", () => {
    ...
    it("should preserve hash fragment in absolute URL", async () => {
      fetchMock.mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
      await fetchInternal("https://other.com/path#section", undefined, undefined, {
        baseURL: "https://api.example.com",
        headers: undefined,
        throwOnFetchError: true,
        throwOnValidationError: true,
      });
      expect(fetchMock).toHaveBeenCalledWith("https://other.com/path#section", expect.any(Object));
    });
    ...
    it("should preserve hash fragment with query params in absolute URL", async () => {
      fetchMock.mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
      await fetchInternal("https://other.com/path?foo=bar#section", undefined, undefined, {
        baseURL: "https://api.example.com",
        headers: undefined,
        throwOnFetchError: true,
        throwOnValidationError: true,
  ...
  describe("URLs with multiple question marks", () => {
    ...
    it("should handle relative URL with multiple ? characters", async () => {
      fetchMock.mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
      await fetchInternal("/path?key=value?with?question?marks", undefined, undefined, {
        baseURL: "https://api.example.com",
        headers: undefined,
        throwOnFetchError: true,
        throwOnValidationError: true,
  ...
  describe("edge cases", () => {
    ...
    it("should handle URL with empty hash", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith("https://api.example.com/path#", expect.any(Object));
    });
    ...
    it("should handle protocol-relative URL with hash", async () => {
      fetchMock.mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
      await fetchInternal("//other.com/path?query=1#section", undefined, undefined, {
        baseURL: "https://api.example.com",
        headers: undefined,
        throwOnFetchError: true,
        throwOnValidationError: true,
    ...
    it("should handle just a path with no base URL and hash", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith("/api/users#top", expect.any(Object));
    });
