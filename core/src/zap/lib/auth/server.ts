...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      const { canSend, timeLeft } = await orpcClient.mails.canSendMail({
        userId: user.id,
      });
      if (!canSend) {
        throw new Error(
      ...
      await orpcClient.mails.sendForgotPasswordMail({
        recipients: [user.email],
        subject: `${SETTINGS.MAIL.PREFIX} - Reset your password`,
        url,
      });

      await orpcClient.mails.updateLastTimestampMailSent({ userId: user.id });
    },
  },
  emailVerification: {
    sendVerificationEmail: async ({ user, url }) => {
      ...
      await orpcClient.mails.sendVerificationMail({
        recipients: [user.email],
